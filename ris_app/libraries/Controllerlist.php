...
class ControllerList {
	...
	private $unwanted_controllers;
	private $unwanted_methods;
	...
	function __construct() {
		...
		//Unwanted Controller
		$this->unwanted_controllers = array('ajax', 'api', 'authenticate', 'cronjobs','dashboard', 'welcome');
		
		//Unwanted Methods
		$this->unwanted_methods = array('getJsonData', 'index');
		...
		foreach(glob(APPPATH . 'controllers/*') as $controller) {
			...
			if(is_dir($controller)) {
				...
				foreach(glob(APPPATH . 'controllers/'.$dirname.'/*') as $subdircontroller) {
					...
					if(in_array($subdircontrollername, $this->unwanted_controllers)){
						continue;
					}
					...
					foreach($aMethods as $method) {
						if($method != '__construct' && $method != 'get_instance' && $method != $subdircontrollername && !in_array($method, $this->unwanted_methods)) {
							$aUserMethods[] = $method;
						}
					}
			    ...
			    if(in_array($controllername, $this->unwanted_controllers)){
					continue;
				}
				...
				if(is_array($aMethods)){
					foreach($aMethods as $method) {
						if($method != '__construct' && $method != 'get_instance' && $method != $controllername && !in_array($method, $this->unwanted_methods)) {
							$aUserMethods[] = $method;
						}
					}
				}
